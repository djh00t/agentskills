...
- Use `scripts/spec_kit_emit_issues.py` allowlists for repository targets.
...
Use `scripts/spec_kit_plan.py` for deterministic dependency ordering and
parallel batch generation directly from skill input JSON.

Use `scripts/spec_kit_emit_issues.py` to emit deterministic `gh` issue-create
commands and publish only when explicit approval is provided.
