...
def build_parser() -> argparse.ArgumentParser:
    """Build argument parser for user timeline retrieval."""
    parser = argparse.ArgumentParser(description="Retrieve posts from a specific user")
    ...
    parser.add_argument("--exclude", help="comma list: replies,retweets")
...
def main() -> None:
    """Run user timeline retrieval and emit result pages."""
    ...
    try:
        ...
        iterator = client.users.get_posts(
            id=args.user_id,
            since_id=args.since_id,
            until_id=args.until_id,
            max_results=args.max_results,
            exclude=parse_csv_option(args.exclude),
        ...
        fail("User post retrieval failed", details=str(err), code=1)
