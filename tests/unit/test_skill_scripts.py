...
def test_spec_kit_skill_has_scripts_directory() -> None:
    """Ensures spec-kit skill includes a scripts directory and planner script."""
    repo_root = Path(__file__).resolve().parents[2]
    scripts_root = repo_root / "skills" / "spec-kit" / "scripts"

    assert scripts_root.is_dir()
    assert (scripts_root / "spec_kit_plan.py").is_file()
    assert (scripts_root / "spec_kit_emit_issues.py").is_file()
    assert (scripts_root / "install_spec_kit_deps.sh").is_file()


def test_spec_kit_planner_script_is_deterministic(tmp_path: Path) -> None:
    """Ensures planner script emits stable deterministic ordering and batches."""
    repo_root = Path(__file__).resolve().parents[2]
    script_path = repo_root / "skills" / "spec-kit" / "scripts" / "spec_kit_plan.py"
...
def test_spec_kit_emit_script_dry_run(tmp_path: Path) -> None:
    """Ensures issue emission script produces deterministic dry-run commands."""
    repo_root = Path(__file__).resolve().parents[2]
    script_path = (
        repo_root / "skills" / "spec-kit" / "scripts" / "spec_kit_emit_issues.py"
    )
...
def test_spec_kit_emit_script_publish_requires_approval(tmp_path: Path) -> None:
...
def test_spec_kit_emit_script_publish_requires_allowlist(tmp_path: Path) -> None:
